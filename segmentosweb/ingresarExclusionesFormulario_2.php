<?PHP														
	...
	validaTiempo();
	if(!isset($_POST["cargador"]) and !isset($_GET["id"])){
		header("Location: verExclusion.php");
	}	
	else if(!isset($_POST["cargador"]) and isset($_GET["id"])){	 
		//Se carga la página
		...
		$consulta = "select count('rolDesafeccionReal') as cantidad_exclusiones from desafeccionreal";
		$resultado = $conexion_db->query($consulta);
		$fila = $resultado->fetch_array(MYSQL_ASSOC);
		if($fila["cantidad_exclusiones"] == 0){
			$tpl->assign("CONTROLAR", "<input type='checkbox' name='no_considerar_fecha'/>"); 
		}
		else{
			$consulta2 = "select sum(`noConsiderarFecha`) as cantidad_considera from desafeccionreal";
			$resultado2 = $conexion_db->query($consulta2);
			$fila2 = $resultado2->fetch_array(MYSQL_ASSOC);		
			//Caso 2: Considera
			if($fila2["cantidad_considera"] == 0){
				$tpl->assign("CONTROLAR", "<input type='checkbox' name='no_considerar_fecha' readonly='readonly' onclick='javascript: return false;'/>"); 
			}
			//Caso 3: No considera
			else{
				$tpl->assign("CONTROLAR", "<input type='checkbox' name='no_considerar_fecha' checked readonly='readonly' onclick='javascript: return false;'/>"); 
			}			
		}		
		...
		for($i=0;$i<$_GET["id"];$i++){			
			...
			$tpl->assign("KM_INICIO", "");
			$tpl->assign("KM_FINAL", "");
			$tpl->assign("LONGITUD", "");
			$tpl->assign("FECHA_INICIO", "");
			$tpl->assign("FECHA_TERMINO", "");				
			$tpl->assign("CHECK", "");
			$tpl->assign("RESOLUCION", "");
			
			$tpl->assign("VALORFAJASELECT", "");
			$tpl->assign("VALORFAJA1", "SI");
			$tpl->assign("VALORFAJA2", "NO");					
						
			$tpl->assign("VALORSANEAMIENTOSELECT", "");
			$tpl->assign("VALORSANEAMIENTO1", "SI");
			$tpl->assign("VALORSANEAMIENTO2", "NO");					
			
			$tpl->assign("VALORCALZADASELECT", "");
			$tpl->assign("VALORCALZADA1", "SI");
			$tpl->assign("VALORCALZADA2", "NO");				
			
			$tpl->assign("VALORBERMASELECT", "");
			$tpl->assign("VALORBERMA1", "SI");
			$tpl->assign("VALORBERMA2", "NO");					
			
			$tpl->assign("VALORDEMARCACIONSELECT", "");
			$tpl->assign("VALORDEMARCACION1", "SI");
			$tpl->assign("VALORDEMARCACION2", "NO");					
			
			$tpl->assign("VALORSENALIZACIONSELECT", "");
			$tpl->assign("VALORSENALIZACION1", "SI");
			$tpl->assign("VALORSENALIZACION2", "NO");					
