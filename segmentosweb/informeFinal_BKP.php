<?PHP														//-----
	...
	if($filaSorteo["cantidadSorteo"] == 0){
		...
		if(isset($_GET["id"])){			
			...
			if($fila8["cantidadKmDescontados"] != 0){
			...
			else{	//No hay información e segmentos descontados
				//Tomamos los bimestres ya sorteados
				...
				if($fila26["ctdadkmoriginales"] == 0){					
					//Insertamos FAJA
					$consulta27 = "insert into kmcontratados (idKmContratados, codigo, operaciones, unidad, cantidad, kmBimestre) ". "values ('', '".$Ccomponente[0]."', 'Faja vial', 'km', ".number_format($fajaDescontado_original, 3, '.', '').", ".$_GET["id"].")";
					$resultado27 = $conexion_db->query($consulta27);
					//Insertamos SANEAMIENTO
					$consulta28 = "insert into kmcontratados (idKmContratados, codigo, operaciones, unidad, cantidad, kmBimestre) ". "values ('', '".$Ccomponente[1]."', 'Saneamiento', 'km',".number_format($saneamientoDescontado_original, 3, '.', '').", ".$_GET["id"].")";
					$resultado28 = $conexion_db->query($consulta28);
					//Insertamos CALZADA
					$consulta29 = "insert into kmcontratados (idKmContratados, codigo, operaciones, unidad, cantidad, kmBimestre) values ('', '".$Ccomponente[2]."', 'Calzada', 'km', ".number_format($calzadaDescontado_original, 3, '.', '').", ".$_GET["id"].")";
					$resultado29 = $conexion_db->query($consulta29);					
					//Insertamos BERMAS
					$consulta30 = "insert into kmcontratados (idKmContratados, codigo, operaciones, unidad, cantidad, kmBimestre) values ('', '".$Ccomponente[3]."', 'Bermas', 'km', ".number_format($bermaDescontado_original, 3, '.', '').", ".$_GET["id"].")";
					$resultado30 = $conexion_db->query($consulta30);										
					//Insertamos SENALIZACION
					...
					number_format($senalizacionDescontado_original, 3, '.', '').", ".$_GET["id"].")";
					$resultado31 = $conexion_db->query($consulta31);											
					//Insertamos DEMARCACION
					$consulta32 = "insert into kmcontratados (idKmContratados, codigo, operaciones, unidad, cantidad, kmBimestre) values ('', '".$Ccomponente[5]."', 'Demarcaci&oacute;n', 'km', ".number_format($demarcacionDescontado_original, 3, '.', '').", ".$_GET["id"].")";
					$resultado32 = $conexion_db->query($consulta32);											
				}
				else{
					//Actualizamos FAJA
					$consulta33 = "update kmcontratados set cantidad = ".number_format($fajaDescontado_original, 3, '.', '').
					" where codigo = '".$Ccomponente[0]."' and kmBimestre = ".$_GET["id"];
					$resultado33 = $conexion_db->query($consulta33);
					//Actualizamos SANEAMIENTO
					$consulta34 = "update kmcontratados set cantidad = ".number_format($saneamientoDescontado_original, 3, '.', '').
					" where codigo = '".$Ccomponente[1]."' and kmBimestre = ".$_GET["id"];
					$resultado34 = $conexion_db->query($consulta34);
					//Actualizamos CALZADA
					$consulta35 = "update kmcontratados set cantidad = ".number_format($calzadaDescontado_original, 3, '.', '').
					" where codigo = '".$Ccomponente[2]."' and kmBimestre = ".$_GET["id"];
					$resultado35 = $conexion_db->query($consulta35);
					//Actualizamos BERMAS
					$consulta36 = "update kmcontratados set cantidad = ".number_format($bermaDescontado_original, 3, '.', '').
					" where codigo = '".$Ccomponente[3]."' and kmBimestre = ".$_GET["id"];
					$resultado36 = $conexion_db->query($consulta36);
					//Actualizamos SENALIZACION
					$consulta37 = "update kmcontratados set cantidad = ".number_format($senalizacionDescontado_original, 3, '.', '').
					" where codigo = '".$Ccomponente[4]."' and kmBimestre = ".$_GET["id"];
					$resultado37 = $conexion_db->query($consulta37);
					//Actualizar DEMARCACION
					$consulta38 = "update kmcontratados set cantidad = ".number_format($demarcacionDescontado_original, 3, '.', '').
					" where codigo = '".$Ccomponente[5]."' and kmBimestre = ".$_GET["id"];
					$resultado38 = $conexion_db->query($consulta38);			
				}
